...
dat <- fread("data/typhoid_outbreak_summary_v1.csv")

ts <- fread("data/typhoid_outbreak_ts_v2.csv")
...
dat <- dat %>%
  filter(!study_id %in% c(
    "Cherian 2015", "Wang 2022 (2)", "Imanishi 2014 (total)",
...
dat <- dat %>%
  ...
  select(
    study_id, country_iso, location, WHO_region, outbreak_start_date, outbreak_end_date,
...
amr1 <- amr %>%
  select(
    study_id, amr_status, tot_tested_amr, tot_amr_cases, resistant, mdr, fqns, xdr,
...
ts <- ts %>%
  group_by(study_id) %>%
  mutate(
    cum_cases = sum(total_cases),
    prop_cases = total_cases / cum_cases,
    deaths = tot_deaths * prop_cases
  ) # use distribution of cases time series for deaths
...
ts <- ts %>%
  rename(
    suspected_cases2 = suspected_cases,
